  ...
  outputs = {
      ...
      forAllSystems = function:
        nixpkgs.lib.genAttrs [
          ...
          "aarch64-darwin"
            ...
            rusty_experiments = craneLib.buildPackage (craneCommon
              // {
    ...
    in {
      checks = forAllSystems ({
        ...
        inherit rusty_experiments;

        rusty_experiments-clippy = craneLib.cargoClippy (craneCommon
          // {
            inherit cargoArtifacts;
        ...
        # Check formatting
        rusty_experiments-fmt = craneLib.cargoFmt {
          inherit src;
        };

        # Audit dependencies
        rusty_experiments-audit = craneLib.cargoAudit {
      ...
      packages = forAllSystems ({ rusty_experiments, ... }: {
        inherit rusty_experiments;
        default = rusty_experiments;
      });

      apps = forAllSystems ({system, ...}: {
        rusty_experiments = {
          type = "app";
          program = "${self.packages.${system}.rusty_experiments}/bin/rusty_experiments";
        };
        default = self.apps.${system}.rusty_experiments;
      });
      ...
      devShells = forAllSystems ({
        ...
        devshell = {
          name = "rusty_experiments-devshell";
          packages = with pkgs; [
            # Rust build inputs
            clang
            coreutils
